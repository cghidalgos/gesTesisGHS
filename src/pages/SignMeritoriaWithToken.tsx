...
export default function SignMeritoriaWithToken() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const res = await fetch(`http://localhost:4000/sign/meritoria/token/${token}`);
  ...
  const handleDownloadPdf = async () => {
    ...
    setLoadingPdf(true);
    try {
      const res = await fetch(`http://localhost:4000/theses/${data.thesisId}/meritoria/download-for-signing?format=pdf`);
      if (!res.ok) throw new Error('Error descargando PDF');
      const blob = await res.blob();
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = `carta-meritoria-${data.thesisId}.pdf`;
      a.click();
  ...
  const handleUploadPdf = async () => {
    ...
    try {
      const res = await fetch(`http://localhost:4000/sign/meritoria/token/${token}/upload-signed`, {
        method: 'POST',
        body: formData,
      });
  ...
  return (
    <div className="max-w-2xl mx-auto p-6 bg-white rounded-lg shadow">
      <h1 className="text-3xl font-bold mb-2">🔐 Firma Digital - Carta de Recomendación Meritoria</h1>
      ...
      <div className="mb-6">
        ...
        <p className="text-sm text-gray-600 mb-4">Por favor descargue, firme y suba el PDF.</p>
      </div>
